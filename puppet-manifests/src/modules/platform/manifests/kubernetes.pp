...
class platform::kubernetes::master::init
  ...
  if str2bool($::is_initial_config_primary) {
    ...
    -> exec { 'restrict kube-dns to master nodes':
      command   => join(['kubectl --kubeconfig=/etc/kubernetes/admin.conf',
                          '-n kube-system patch deployment kube-dns -p',
    ...
    -> exec { 'remove taint from master node':
      command   => join(['kubectl --kubeconfig=/etc/kubernetes/admin.conf',
                          "taint node ${::platform::params::hostname} node-role.kubernetes.io/master-"], ' '),
      logoutput => true,
    }
  } else {
    if str2bool($::is_initial_config) {
      ...
      -> exec { 'restrict kube-dns to master nodes':
        command   => join(['kubectl --kubeconfig=/etc/kubernetes/admin.conf',
                            '-n kube-system patch deployment kube-dns',
                            '-p \'{"spec":{"template":{"spec":{"nodeSelector":{"node-role.kubernetes.io/master":""}}}}}\''], ' '),
        logoutput => true,
      }
