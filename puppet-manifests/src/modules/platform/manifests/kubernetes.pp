...
class platform::kubernetes::cgroup
  ...
  $controllers.each |String $controller| {
    $parent_dir = "${cgroup_root}/${controller}"
    $cgroup_dir = "${parent_dir}/${cgroup_name}"
    ...
    if $controller == 'cpuset' {
      ...
      # Create machine.slice cgroup to be used with VMs.
      # This prevents the machine.slice cpuset from inheriting the
      # kubernetes libvirt pod cpuset.
      $parent_mems = "${parent_dir}/cpuset.mems"
      $parent_cpus = "${parent_dir}/cpuset.cpus"
      $machine_dir = "${parent_dir}/${cgroup_machine}"
