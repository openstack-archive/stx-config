class platform::kubernetes::params (
  ...
  # TODO(jgauld): This duplicates host_labels from
  # source: https://review.openstack.org/#/c/646038/
  $host_labels = [],
  $ca_crt = undef,
  $ca_key = undef,
  $sa_key = undef,
...
class platform::kubernetes::cgroup::cpuset
  ...
  if !defined('$k8s_cpuset') {
      $k8s_cpuset = generate('/bin/bash', '-c', 'cat /sys/devices/system/cpu/online')
      notice("System default cpuset ${k8s_cpuset}.")
  }
  if !defined('$k8s_nodeset') {
      $k8s_nodeset = generate('/bin/bash', '-c', 'cat /sys/devices/system/node/online')
