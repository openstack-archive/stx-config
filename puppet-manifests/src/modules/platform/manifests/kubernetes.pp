...
class platform::kubernetes::cgroup
  ...
  $controllers.each |String $controller| {
    $cgdir = "${cgroup_root}/${controller}/${cgroup_name}"

    File[ $cgroup_root ]
    ->file { "${cgroup_root}/${controller}":
      ensure => present,
      owner  => 'root',
      group  => 'root',
      mode   => '0755',
    ...
    # tag to update contents under /sys, since puppet tries to create
    # files with temp names in the same directory.
