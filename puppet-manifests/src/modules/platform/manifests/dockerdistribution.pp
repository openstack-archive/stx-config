  ...
  if $enabled {
    ...
    if str2bool($::is_initial_config_primary) {
      ...
      exec { "docker-registry-generate-cert":
        command => "openssl req -x509 -sha256 -nodes -days 365 -newkey rsa:2048 -keyout ${certs_dir}/registry-cert.key -out ${certs_dir}/registry-cert.crt -config ${certs_dir}/registry-cert-extfile.cnf"
      } ->

      exec { "docker-registry-generate-pkcs1-cert-from-pkcs8":
        command => "openssl rsa -in ${certs_dir}/registry-cert.key -out ${certs_dir}/registry-cert-pkcs1.key"
      } ->
