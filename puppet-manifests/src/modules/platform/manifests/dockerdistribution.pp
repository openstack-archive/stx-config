...
class platform::dockerdistribution::config
  ...
  if $enabled {
    ...
    # this needs to be generated here because the certificate need to know the registry ip address for SANs
    if str2bool($::is_initial_config_primary) {
      $sw_version = $::platform::params::software_version
      $shared_dir = "/opt/platform/config/${sw_version}"

      # create the certificate files
      file { "/etc/ssl/private/registry-cert-extfile.cnf":
        ensure  => present,
        owner   => 'root',
        group   => 'root',
        mode    => '0400',
      ...
      exec { "docker-registry-generate-cert":
        command => "openssl req -x509 -sha256 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/private/registry-cert.key -out /etc/ssl/private/registry-cert.crt -config /etc/ssl/private/registry-cert-extfile.cnf"
      } ->

      exec { "docker-registry-generate-pkcs1-cert-from-pkcs8":
        command => "openssl rsa -in /etc/ssl/private/registry-cert.key -out /etc/ssl/private/registry-cert-pkcs1.key"
      } ->

      # ensure permissions are set correctly
      file { '/etc/ssl/private/registry-cert-pkcs1.key':
        ensure => 'file',
        owner => 'root',
        group => 'root',
        mode => '0400',
      ...
      file { '/etc/ssl/private/registry-cert.key':
        ensure => 'file',
        owner => 'root',
        group => 'root',
        mode => '0400',
      ...
      file { '/etc/ssl/private/registry-cert.crt':
        ensure => 'file',
        owner => 'root',
        group => 'root',
        mode => '0400',
      ...
      exec { "remove-registry-cert-extfile":
        command => "rm /etc/ssl/private/registry-cert-extfile.cnf"
      } ->

      # copy certificates and keys to shared directory for second controller
      # we do not need to worry about second controller being up at this point, since we have a is_initial_config_primary check
      file { "${shared_dir}/registry-cert-pkcs1.key":
        ensure => 'file',
        owner => 'root',
        group => 'root',
        mode => '0400',
        source => '/etc/ssl/private/registry-cert-pkcs1.key',
      } ->
      ...
      file { "${shared_dir}/registry-cert.key":
        ...
        group => 'root',
        mode => '0400',
        source => '/etc/ssl/private/registry-cert.key',
      } ->
      ...
      file { "${shared_dir}/registry-cert.crt":
        ...
        group => 'root',
        mode => '0400',
        source => '/etc/ssl/private/registry-cert.crt',
      } ->

      # copy the certificate to docker certificates directory, which makes docker trust that specific certificate
      # this is required for self-signed and also if the user does not have a certificate signed by a "default" CA
      file { "/etc/docker":
        ensure  => 'directory',
        owner   => 'root',
        group   => 'root',
