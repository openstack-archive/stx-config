...
class platform::multipath
  ...
  if $enabled {
    ...
    exec { 'systemctl-start-multipathd':
      command => '/usr/bin/systemctl start multipathd.service',
    }
  } else {
    ...
    exec { 'systemctl-stop-multipathd':
      command => '/usr/bin/systemctl stop multipathd.service',
    } ->
