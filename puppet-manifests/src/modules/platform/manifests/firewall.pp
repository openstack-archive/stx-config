...
class platform::firewall::calico::oam::policies {
  include ::platform::params
  include ::platform::network::oam::params
  ...
  if $::platform::params::distributed_cloud_role == 'systemcontroller' {
    $dcmanager_port = [$::platform::dcmanager::params::api_port]
    $sysinv_api_proxy_port = [$::platform::dcorch::params::sysinv_api_proxy_port]
    $patch_api_roxy_port = [$::platform::dcorch::params::patch_api_proxy_port]
    $identify_api_proxy_port = [$::platform::dcorch::params::identity_api_proxy_port]
  } else {
    $dcmanager_port = []
    $sysinv_api_proxy_port = []
    $patch_api_roxy_port = []
    $identify_api_proxy_port = []
  }
  ...
  $t_tcp_ports = concat($ssh_port,
                        ...
                        $dcmanager_port,
                        $sysinv_api_proxy_port, $patch_api_roxy_port,$identify_api_proxy_port)
  $t_order = 100
  notice("t_ip_version: ${t_ip_version}")
  notice("t_icmp_proto: ${t_icmp_proto}")
  notice("t_igmp_proto: ${t_igmp_proto}")
  notice("t_udp_ports: ${t_udp_ports}")
  notice("t_tcp_ports: ${t_tcp_ports}")
  notice("t_order: ${t_order}")

  $file_name = '/tmp/gnp_all_oam.yaml'
  file { $file_name:
      ensure  => file,
      content => template('platform/calico_oam_gnp.yaml.erb'),
      owner   => 'root',
      group   => 'root',
      mode    => '0640',
  }
...
class platform::firewall::calico::oam::endpoints {
  ...
  $file_name = "/tmp/hep_${host}_oam.yaml"
  file { $file_name:
    ensure  => file,
...
class platform::firewall::calico::oam {
  ...
  contain ::platform::firewall::calico::oam::policies
