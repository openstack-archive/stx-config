...
class platform::sm
  ...
  if $region_config {
    ...
    if $::platform::params::distributed_cloud_role =='subcloud' {
      ...
      -> exec { 'Provision DCOrch-DBsync-RestApi (service-group-member dbsync-api)':
        command => "sm-provision service-group-member distributed-cloud-services dbsync-api",
      }
      -> exec { 'Provision DCOrch-DBsync-RestApi in SM (service dbsync-api)':
        command => "sm-provision service dbsync-api",
      }
       -> exec { 'Configure OpenStack - DCOrch-DBsync-API':
        command => "sm-configure service_instance dbsync-api dbsync-api \"\"",
      }
