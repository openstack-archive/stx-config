...
class openstack::cinder
  ...
  $internal_project_id = generate('/bin/bash', '-c', 'source /etc/nova/openrc && openstack project create cinder \\
    --domain= --or-show -f value -c id ')
  $internal_user_id = generate('/bin/bash', '-c', 'source /etc/nova/openrc && openstack user create cinder \\
    --password "Madawaska1*" --domain= --or-show -f value -c id ')
