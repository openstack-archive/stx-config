...
@utils.arg('hostnameorid',
           metavar='<hostname or id>',
           help='TODO: hostname for compute')
def do_ceph_mon_add(cc,args):
    ihost = ihost_utils._find_ihost(cc, args.hostnameorid)

    fields = {}

    fields['forihostid'] = ihost.id

    try:
        ceph_mon = cc.ceph_mon.create(**fields)
    except exc.HTTPNotFound:
        raise exc.CommandError(
            "Physical volume creation failed: host %s: "
             % args.hostnameorid)
    ...
    try:
        ...
        #TODO: uncomment raise and delete print
        print "Created ceph monitor UUID not found: %s" % suuid
...
def do_ceph_mon_delete(cc,args):
    # TODO: delete this. This is for easier testing
    ihost = ihost_utils._find_ihost(cc, args.hostnameorid)
    fields = {}
    fields['forihostid'] = ihost.id

    try:
        cc.ceph_mon.delete(ihost.id)
    except exc.HTTPNotFound:
