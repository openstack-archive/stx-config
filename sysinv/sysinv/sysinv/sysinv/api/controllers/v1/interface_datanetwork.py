...
# Can only assign the following data network types
ASSIGNABLE_DATANETWORKTYPES = (
    ...
    def convert_with_links(cls, rpc_interface_datanetwork, expand=True):
        ...
        if not expand:
            interface_datanetwork.unset_fields_except([
                ...
                'datanetwork_name', 'datanetwork_type'
            ])
...
class InterfaceDataNetworkController(rest.RestController):
    ...
    def _create_interface_datanetwork(self, interface_datanetwork):
        ...
        datanetwork_obj = pecan.request.dbapi.datanetwork_get(datanetwork_uuid)
        datanetwork_id = datanetwork_obj['id']
        datanetwork_type = datanetwork_obj['datanetwork_type']
        ...
        self._check_assigned_datanetwork_type(datanetwork_type)
    ...
    def _check_interface_mtu(interface_obj, datanetwork_obj):
        if interface_obj.imtu < datanetwork_obj.mtu:
            msg = _("The interface MTU %s must be larger than '%s' "
                    "datanetwork MTU." %
    ...
    @staticmethod
    def _check_assigned_datanetwork_type(datanetwork_type):
        if datanetwork_type in ASSIGNABLE_DATANETWORKTYPES:
            return
        raise exception.UnsupportedAssignedInterfaceDataNetworkType(
            datanetwork_type=datanetwork_type)
    ...
    def _get_datanetwork_id_and_type(datanetwork_uuid):
        ...
        return datanetwork['id'], datanetwork['datanetwork_type']
