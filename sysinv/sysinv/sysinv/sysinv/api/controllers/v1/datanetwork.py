    ...
    def convert_with_links(cls, rpc_datanetwork, expand=True):
        ...
        if not expand:
            datanetwork.unset_fields_except(
                ['id', 'uuid', 'datanetwork_type', 'name',
                 'description', 'mtu',
                 'multicast_group', 'port_num', 'ttl', 'mode'])

        return datanetwork

    def _validate_datanetwork_type(self):
        if self.datanetwork_type not in ALLOWED_DATANETWORK_TYPES:
            raise ValueError(_("DataNetwork type %s not supported") %
                             self.datanetwork_type)

    def validate_syntax(self):
        """
        Validates the syntax of each field.
        """
        self._validate_datanetwork_type()
...
class DataNetworkController(rest.RestController):
    ...
    @staticmethod
    def _check_datanetwork_type(datanetwork):
        if 'datanetwork_type' not in datanetwork:
            raise wsme.exc.ClientSideError(
                _('DataNetwork datanetwork_type is required.'))

        datanetwork_type = datanetwork['datanetwork_type']
        if datanetwork_type not in ALLOWED_DATANETWORK_TYPES:
    ...
    def _check_datanetwork_vxlan(datanetwork):
        if datanetwork['datanetwork_type'] != constants.DATANETWORK_TYPE_VXLAN:
            return
    ...
    def _check_datanetwork(self, datanetwork):
        self._check_datanetwork_type(datanetwork)
