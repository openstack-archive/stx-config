...
#        ceph_mon = objects.ceph_mon.get_by_uuid(
#            pecan.request.context,
#            host_id)
        _delete(host_id)
...
def _create(ceph_mon):
    ...
    # is on a compute node, so we check for that
    if chost.personality == constants.COMPUTE:
...
def _delete(host_id):
    existing_ceph_mon = pecan.request.dbapi.ceph_mon_get_by_ihost(
        host_id)
    try:
        pecan.request.dbapi.ceph_mon_destroy(existing_ceph_mon[0].uuid)
    except exception.HTTPNotFound:
        raise wsme.exc.ClientSideError("failed deleting ceph_mon")
