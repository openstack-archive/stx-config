...
def _create(ceph_mon):
    ...
    try:
        ...
        raise wsme.exc.ClientSideError(
            _("Host not found id: %s ." % ceph_mon['forihostid']))
    ...
    if chost['personality'] == constants.WORKER:
        # Only if replication model is CONTROLELR or not yet defined
        ...
        if stor_model not in worker_stor_models:
            raise wsme.exc.ClientSideError(
                _("Can not add a storage monitor to a worked if "
        ...
        suppoted_replication = constants.CEPH_CONTROLLER_MODEL_REPLICATION_SUPPORTED
        if replication not in suppoted_replication:
            raise wsme.exc.ClientSideError(
                _("Ceph monitor can be added to a worker only if "
                  "replication is set to: %s'. Please update replication "
                  "before configuring a monitor on a worker node." % suppoted_replication))
