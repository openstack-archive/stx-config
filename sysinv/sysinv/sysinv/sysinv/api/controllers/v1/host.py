...
class HostController(rest.RestController):
    ...
    def delete(self, ihost_id):
        ...
        # tell conductor to delete the barbican entry associatedi with this host (if present)
        if not pecan.request.rpcapi.delete_barbican_secret(
                                               pecan.request.context,
                                               ihost.uuid):
            LOG.warn(_("No response from barbican delete api %s" % ihost.hostname))
    ...
    def _bm_semantic_check_and_update(self, ohost, phost, delta, patch_obj,
        ...
        if password_exists and patch_bm_password:
            if not pecan.request.rpcapi.create_barbican_secret(pecan.request.context,
                                                            phost['uuid'],
                                                            patch_bm_password):
                LOG.warn(_("No response from barbican create api %s" % phost['hostname']))
