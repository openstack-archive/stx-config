...
class HostController(rest.RestController):
    ...
    def _semantic_check_filesystem_sizes(ihost):
        ...
        if utils.is_host_active_controller(ihost):
            return
        ...
        if (ihost['invprovision'] and
            ...
            LOG.info("Controller host %s already provisioned. Skipping "
                     "filesystem checks." % ihost['hostname'])
            return
        ...
        if (active_controller_used > standby_controller_allocated_space):
            ...
            msg = _("Standby controller has not enough space allocated to "
