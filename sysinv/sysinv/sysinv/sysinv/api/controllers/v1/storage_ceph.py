...
def _discover_and_validate_rbd_provisioner_capabilities(caps_dict, storage_ceph):
    # Use same regex that Kubernates uses to validate its labels
    ...
    if constants.K8S_RBD_PROV_NAMESPACES in caps_dict:
        ...
        for namespace in namespaces:
            if not r.match(namespace):
                msg = ("Invalid list of namespaces provided: '%s' please "
                       "provide a valid comma separated list of Kubernates "
    ...
    if constants.K8S_RBD_PROV_STORAGECLASS_NAME in caps_dict:
        ...
        if not r.match(name):
                msg = ("Invalid RBD StorageClass name '%s'. %s" % (name, msg_help))
                raise wsme.exc.ClientSideError(msg)

        # Check the unicity of RBD StorageClass name in DB.
        if constants.K8S_RBD_PROV_STORAGECLASS_NAME in caps_dict:
            ...
            if name in storclass_names:
                msg = ("RBD StorageClass name '%s'is already used by another backend." % name)
                raise wsme.exc.ClientSideError(msg)
...
def _check_backend_ceph(req, storage_ceph, confirmed=False):
    ...
    for svc in req_services:
        ...
        # Service is valid. Discover the latest config data for the supported service
        discover_func = eval(
...
def validate_k8s_namespaces(values):
    """ Check if a list of namespaces is configured in Kubernates """
    ...
    if invalid_namespaces:
        msg = _("Error configuring rbd-provisioner service. "
                "The following Kubernates namespaces are not "
