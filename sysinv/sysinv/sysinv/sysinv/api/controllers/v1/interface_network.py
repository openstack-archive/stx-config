    ...
    def convert_with_links(cls, rpc_interface_network, expand=True):
        ...
        if not expand:
            interface_network.unset_fields_except([
                'forihostid', 'id', 'uuid',
                'interface_id', 'interface_uuid', 'ifname',
...
class InterfaceNetworkController(rest.RestController):
    ...
    def _create_interface_network(self, interface_network):
        ...
        self._check_duplicate(interface_network_dict)
        self._check_type(network_type)
        self._check_assigned_network_type(network_type)

        result = pecan.request.dbapi.interface_network_create(interface_network_dict)

        # pecan.request.rpcapi.update_interface_network_config(pecan.request.context,
        #                                                      result.forihostid)
    ...
    def _check_duplicate(self, interface_network):
    ...
    def _check_type(self, network_type):
        if network_type in NONDUPLICATE_NETWORK_TYPES:
            interface_networks = pecan.request.dbapi.interface_network_get_all()
            for n in interface_networks:
                if n.network_type == network_type:
    ...
    """
    def _check_removed_network_type(self, network_type):
        if network_type not in NONREMOVABLE_NETWORK_TYPES:
            return
        raise exception.UnsupportedRemovedInterfaceNetworkType(network_type=network_type)
    """
    ...
    def delete(self, interface_network_uuid):
        self._get_one(interface_network_uuid)
        # network_type = getattr(interface_network, 'network_type')
        # self._check_removed_network_type(network_type)
