...
class StorageController(rest.RestController):
    ...
    def patch(self, stor_uuid, patch):
        ...
        if updated and rpc_stor['function'] == constants.STOR_FUNCTION_OSD:
            ihost_id = rpc_stor['forihostid']
            ihost = pecan.request.dbapi.ihost_get(ihost_id)
            if ihost['availability'] == constants.AVAILABILITY_AVAILABLE:
...
def _check_host(stor):
    ...
    if stor_model == constants.CEPH_CONTROLLER_MODEL:
        if (ihost['administrative'] == constants.ADMIN_UNLOCKED and
                (ihost['operational'] != constants.OPERATIONAL_ENABLED or
                 ihost['availability'] != constants.AVAILABILITY_AVAILABLE)):
            msg = _("Host %s must be unlocked, enabled and "
                    "available." % ihost['hostname'])
