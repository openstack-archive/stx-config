...
class InterfaceController(rest.RestController):
    ...
    def patch(self, interface_uuid, patch):
        ...
        patches_to_remove = []
        providernetworks_to_update = None
        for p in patch:
            if '/ifclass' == p['path']:
                ...
                patches_to_remove.append(p)
            elif '/providernetworks' == p['path']:
                providernetworks_to_update = p['value'].split(',')
                patches_to_remove.append(p)

        if uses:
            patch.append(dict(path='/uses', value=uses, op='replace'))

        if not datanetworks and providernetworks_to_update:
            datanetworks = providernetworks_to_update
            LOG.info("providernetworks_to_update=%s" % datanetworks)
        ...
        if datanetworks_to_add:
            for datanetwork_id in datanetworks_to_add:
                ...
                try:
                    ...
                    pass
        elif datanetworks or providernetworks_to_update:
...
def _update_interface_datanetworks(host_uuid, interface,
                                   datanetworks=None,
                                   delete_ifdn=False):
    if not cutils.datanetworks_supported():
        return []
    ...
    if datanetworks:
        for datanetwork_id in datanetworks:
            ...
            pns.append(dn.name)
    elif 'datanetworks' in interface and 'providernetworks' not in interface:
        pns = interface['datanetworks']
    elif 'providernetworks' in interface and interface['providernetworks']:
        pns = interface['providernetworks'].split(",")
    ...
    for pn in pns:
        if pn == constants.DATANETWORK_TYPE_NONE:
            continue
...
def _check_providernetworks(ihost,
                            interface,
                            interface_list,
                            existing_interface,
                            networktypelist):

    """This method is to be removed and replaced by _check_datanetworks()
       when integrated with spec for neutron segment-range management
       https://github.com/openstack/neutron-specs/blob/
       master/specs/stein/network-segment-range-management.rst
    """
    if 'id' in interface:
        this_interface_id = interface['id']
    else:
...
def _check_datanetworks(ihost,
    ...
    # else:
    #     datanetworks_names = interface.get('providernetworks')
    #     if interface.get('providernetworks'):
    #         datanetworks = datanetworks_names.split(",")

    LOG.info("_check_datanetworks datanetworks interface=%s datanetworks=%s" %
    ...
    for db in db_datanetworks:
        all_datanetworks[db.name] = {
            'datanetwork_type': db.datanetwork_type}
    ...
    if interface['ifclass'] in NEUTRON_INTERFACE_CLASS:
        ...
        for pn in [n.strip() for n in datanetworks_list]:
            ...
            # can be assigned is VLAN.
            if (datanet['datanetwork_type'] != constants.DATANETWORK_TYPE_VLAN and
                    ifclass not in NEUTRON_NETWORK_TYPES):
                msg = _("Data network '%s' of type '%s' cannot be assigned "
                        "to an interface with interface class '%s'"
                        % (pn, datanet['datanetwork_type'], ifclass))
                raise wsme.exc.ClientSideError(msg)
