...
class Interface(base.APIBase):
    ...
    ifclass = wtypes.text
    "Represent the class of the iinterface"

    networktype = wtypes.text
    "Represent the network type of the iinterface"

    network_uuid = types.uuid
    "The uuid of the Network associated with the iinterface"
...
class InterfaceController(rest.RestController):
    ...
    def patch(self, interface_uuid, patch):
        ...
        # The variable 'networks' contains a list of networks that the interface
        # should have by the end if this update. These should be
        ...
        try:
            ...
            LOG.exception("Failed to create network interface association: "
                          "new_interface={} interface={}".format(rpc_interface, interface))
            raise e
    ...
    try:
        if (new_interface['ifclass'] and
            ...
            if 'networks' in interface.keys() and interface['networks']:
                ...
                if 'network_uuid' in interface.keys() and interface['network_uuid']:
                    network = pecan.request.dbapi.network_get(interface['network_uuid'])
                    values = {'interface_id': new_interface['id'],
                              'network_id': network['id']}
                    pecan.request.dbapi.interface_network_create(values)
                elif new_interface['networktype'] in VALID_NETWORK_TYPES:
                    network = pecan.request.dbapi.network_get_by_type(new_interface['networktype'])
                    values = {'interface_id': new_interface['id'],
                              'network_id': network['id']}
                    pecan.request.dbapi.interface_network_create(values)
    except Exception as e:
