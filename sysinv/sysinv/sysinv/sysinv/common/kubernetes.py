...
class KubeOperator(object):
    ...
    def _kube_create_namespace(self, namespace):
        ...
        try:
            ...
            pass
        LOG.error("Fail to create Namespace %s: "
                  "%s" % (namespace, e))
        return False
    ...
    def kube_get_namespace(self, namespace):
        ...
        try:
            ...
            if e.status == httplib.NOT_FOUND:
                if self._kube_create_namespace(namespace):
                    return True
            e = e.body
        except Exception as e:
            pass
        LOG.error("Fail to get Namespace %s: "
                  "%s" % (namespace, e))
        return False
    ...
    def kube_get_secret(self, name, namespace):
        ...
        LOG.error("Failed to get Secret %s under "
                  "Namespace %s: %s" % (name, namespace, e))
        return False
    ...
    def kube_delete_secret(self, name, namespace, **kwargs):
        ...
        if kwargs:
            body.setdefault('metadata', {}).update(kwargs)
