...
class PlatformPuppet(base.BasePuppet):
    ...
    def _get_host_memory_config(self, host):
        ...
        if constants.WORKER in utils.get_personalities(host):
            ...
            # This will be used for check if hugepage to be updated.
            huge_pages_to_be_updated = False
            ...
            for node, memory_list in memory_numa_list.items():
                ...
                # If any user'hugepage request pending ,
                # set True to allow update hugepage.
                if (memory.vm_hugepages_nr_2M_pending is not None or
                        memory.vm_hugepages_nr_1G_pending is not None):
                    huge_pages_to_be_updated = True
            ...
            # If user request pending or has openstack_compute label
            # then allow update.
            if huge_pages_to_be_updated or self.is_openstack_compute(host):
                config.update({
