...
class InterfacePuppet(base.BasePuppet):
    ...
    def _get_floating_ip_index(self):
        ...
        # infra_address = self._get_address_by_name(
        #     constants.CONTROLLER_HOSTNAME, constants.NETWORK_TYPE_INFRA)
        #
        # infra_floating_ip = (str(infra_address.address) + '/' +
        #                      str(infra_address.prefix))
        # floating_ips.update({
        #     constants.NETWORK_TYPE_INFRA: infra_floating_ip,
        # })
...
def generate_dhcp_config(context, config):
    ...
    if not is_controller(context):
        ...
        cluster_host_interface = find_interface_by_type(
            context, constants.NETWORK_TYPE_CLUSTER_HOST)
        if cluster_host_interface:
            cid = utils.get_dhcp_cid(context['hostname'],
                                     constants.NETWORK_TYPE_CLUSTER_HOST,
                                     cluster_host_interface['imac'])
            config['platform::dhclient::params::infra_client_id'] = cid
