...
def get_test_datanetwork(**kw):
    datanetwork = {
        ...
        'name': kw.get('name'),
        'datanetwork_type': kw.get('datanetwork_type', 'vxlan'),
        'mtu': kw.get('mtu', '1500'),
...
def create_test_datanetwork(**kw):
    ...
    if kw['datanetwork_type'] != constants.DATANETWORK_TYPE_VXLAN:
...
def post_get_test_interface(**kw):
    ...
    providernetworks = kw.get('providernetworks') or ""
    if not datanetworks and providernetworks:
        datanetworks = providernetworks
...
def create_test_interface(**kw):
    ...
    if 'providernetworks' in interface:
        del interface['providernetworks']
