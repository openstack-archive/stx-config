...
class Connection(api.Connection):
    ...
    def _interface_create(self, obj, forihostid, values):
        ...
        with _session_for_write() as session:
            ...
            # JKUNG if getattr(obj, 'providernetworks', None) == 'none':
            # JKUNG     setattr(obj, 'providernetworks', None)
    ...
    def _datanetwork_get_one(self, datanetwork_id, datanetwork=None):
        ...
        if datanetwork is not None:
            query = query.filter_by(datanetwork_type=datanetwork)
        try:
            result = query.one()
        except NoResultFound:
            raise exception.DataNetworkNotFound(
    ...
    def datanetwork_create(self, values):
        ...
        datanetwork_type = values.get('datanetwork_type')
        if datanetwork_type == constants.DATANETWORK_TYPE_FLAT:
            datanetwork = models.DataNetworksFlat()
        elif datanetwork_type == constants.DATANETWORK_TYPE_VLAN:
            datanetwork = models.DataNetworksVlan()
        elif datanetwork_type == constants.DATANETWORK_TYPE_VXLAN:
    ...
    def _add_datanetworks_filters(self, query, filters):
        if filters is None:
            filters = dict()
        supported_filters = {'datanetwork_type',
                             'name',
                             }
