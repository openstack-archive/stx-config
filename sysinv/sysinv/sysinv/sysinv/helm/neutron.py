...
class NeutronHelm(openstack.OpenstackBaseHelm):
            ...
            common.HELM_NS_OPENSTACK: {
                ...
                'conf': {
                    ...
                    'plugins': {
                        ...
                        # 'openvswitch_agent':
                        #     self._get_default_per_host_overrides(),
                    },
                    'dhcp_agent': {
                        'DEFAULT': {
                            'resync_interval': 30,
    ...
    def _get_default_per_host_overrides(self):
        """Provide default per-host options, to handle the case where
           helm doesn't correctly support per-host overrides"""
        per_host_overrides = self._get_per_host_overrides()
        if not per_host_overrides:
            return {}
        return per_host_overrides[0]['conf']['plugins']['openvswitch_agent']
