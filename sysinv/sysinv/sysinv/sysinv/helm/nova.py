...
class NovaHelm(openstack.OpenstackBaseHelm):
    ...
    def get_overrides(self, namespace=None):
        ...
        overrides = {
            common.HELM_NS_OPENSTACK: {
                ...
                'conf': {
                    'ceph': {
                        'enabled': self._is_ceph_backend_present(),
                        'ephemeral': self._get_rbd_ephemeral_storage()
                    },
                    'nova': {
                        'libvirt': {
    ...
    def _is_ceph_backend_present(self):
        rc = False
        try:
            self.dbapi.storage_backend_get_by_name(
    ...
    def _get_rbd_ephemeral_storage(self):
        ...
        # Get the primary Ceph backend. If there is none, return an empty
        # dictionary.
        is_ceph_backend_present = self._is_ceph_backend_present()
        if not is_ceph_backend_present:
