...
class AppOperator(object):
    ...
    def _update_app_status(self, app, new_status=None, new_progress=None):
        ...
        if (new_status in [constants.APP_UPLOAD_SUCCESS,
        ...
        def _check_progress(monitor_flag, app, pattern, logfile):
            ...
            try:
                with Timeout(INSTALLATION_TIMEOUT,
                             exception.KubeAppProgressMonitorTimeout()) as timeout:
                    while True:
                        try:
                            ...
                            greenthread.sleep(1)
                    LOG.debug("Canceling monitor timeout for %s" % app.name)
                    timeout.cancel()
            except Exception as e:
    ...
    def perform_app_remove(self, rpc_app):
        ...
        if self._make_armada_request_with_monitor(app, 'delete'):
            if app.system_app:
                try:
