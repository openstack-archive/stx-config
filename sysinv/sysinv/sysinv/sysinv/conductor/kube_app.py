...
class AppOperator(object):
    ...
    def _cleanup(self, app):
        ...
        try:
            # TO-DO (tngo): Disable node labeling for system app for now until
            ...
            if app.system_app and app.status != constants.APP_UPLOAD_FAILURE:
                ...
                os.unlink(os.path.join(common.HELM_OVERRIDES_PATH,
                                       'stx-osh-rbd-override.yaml'))
    ...
    def _extract_tarfile(self, app):
        try:
            ...
            tar = tarfile.open(app.tarfile, 'r:gz')
            tar.extractall(path=app.path)
            ...
            for label_str in labels:
                try:
                    self._dbapi.label_create(
                        host.id, {'host_id': host.id, 'label': label_str})
    ...
    def _get_overrides_files(self, charts):
        ...
        # The rbd-provisioner overrides for kube-system namespace and
        # stx-osh namespace should have been generated.
        #
        # TO-DO (tngo): rbd-provisioner for stx-osh is not in
        # the armada manifest. Copy the overrides file from /root/tools
        # and add it to the list of charts for now.
        shutil.copy('/root/tools/stx-osh-rbd-override.yaml',
                    os.path.join(common.HELM_OVERRIDES_PATH,
                                 'stx-osh-rbd-override.yaml'))
        charts.append(Chart('rbd-override', 'stx-osh'))
    ...
    def perform_app_upload(self, rpc_app, tarfile):
        ...
        try:
            ...
            if os.path.isdir(app.charts_dir):
                ...
                # Temporarily allow read access to /scratch so www user can
                # upload helm charts
        ...
        try:
            ...
            if armada_svc:
                if request == 'validate':
                    ...
                    rc = False
                    LOG.error("Unsupported armada request.")
            else:
