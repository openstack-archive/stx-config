    ...
    def _create_local_registry_secrets(self, app_name):
        ...
        for ns in namespaces:
            if (ns == common.HELM_NS_HELM_TOOLKIT or
                (self._kube.kube_get_namespace(ns) and
        ...
        for ns in namespaces:
            ...
            try:
                self._kube.kube_delete_secret(
                    DOCKER_REGISTRY_SECRET, ns, gracePeriodSeconds=0)
        ...
        if self._make_armada_request_with_monitor(app, constants.APP_DELETE_OP):
            if app.system_app:
                ...
                try:
                    ...
                    self._kube.kube_delete_persistent_volume_claim(common.HELM_NS_OPENSTACK,
                                                                   timeout_seconds=10)
                    LOG.info("Persistent Volume Claim delete completed.")
                    self._kube.kube_delete_namespace(common.HELM_NS_OPENSTACK,
                                                     gracePeriodSeconds=0)
                    LOG.info("Namespace %s delete completed." % common.HELM_NS_OPENSTACK)
                except Exception as e:
