    ...
    def _get_image_tags_by_charts(self, app_images_file, app_manifest_file):
        ...
        for chart in charts:
            ...
            if "armada/Chart/" in chart['schema']:
                ...
                if chart['data']['chart_name'] in images_file:
                    images_charts = images_file[chart['data']['chart_name']]

                # Get the image tags from the overrides file
                overrides = chart['data']['namespace'] + '-' + chart['data']['chart_name'] + '.yaml'
                ...
                try:
                    images_manifest = chart['data']['values']['images']['tags']
                except (TypeError, KeyError):
                    LOG.info("Armada manifest file has no img tags for "
                             "chart %s" % chart['data']['chart_name'])
                    pass
                ...
                if manifest_image_tags_updated:
                    if 'values' in chart['data']:
                        chart["data"]["values"]["images"] = {"tags": images_manifest}
                    else:
                        chart["data"]["values"] = {"images": {"tags": images_manifest}}
