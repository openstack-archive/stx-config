...
class ConductorManager(service.PeriodicService):
    ...
    def get_k8s_namespaces(self, context):
        ...
        try:
            ...
            raise exception.SysinvException(
                "Error getting Kubernates list of namespaces, "
                "Details: %s" % str(e))
    ...
    def check_and_update_rbd_provisioner(self, context, new_storceph=None):
        ...
        To allow delayed executions we check DB entries for changes and only then
        proceed with time consuming modifications.
        ...
        """
        return self._ceph.check_and_update_rbd_provisioner(context, new_storceph)
    ...
    def create_controller_filesystems(self, context, rootfs_device):
        ...
        if disk_size > constants.DEFAULT_SMALL_DISK_SIZE:
            ...
            #   163.0 G - PV for cgts-vg (specified in the kickstart)
            # or
            ...
            #          5 G - /opt/gnocchi
            #          1 G - anchor_lv
            #         10 G - /var/lib/docker (--kubernetes)
            ...
            #      20.0 G - /
            #     163.0 G - cgts-vg PV
            #     316.5 G - unpartitioned free space
            #
            ...
            #
            # Min size of the cgts-vg PV is:
            #   113.0 G - PV for cgts-vg (specified in the kickstart)
            # or
            #   121.0 G - (for DCSC non-AIO)
            #          8 G - /var/log (reserved in kickstart)
            #          8 G - /scratch (reserved in kickstart)
            #          2 G - cgcs_lv (DRBD bootstrap manifest)
            #          2 G - pgsql_lv (DRBD bootstrap manifest)
            ...
            #        -----
            #        131 G (for DCSC non-AIO) or 123 G
            #
            #  The absolute minimum disk size for these default settings:
            #     0.5 G - /boot
            #    20.0 G - /
            #   123.0 G - cgts-vg PV
            # or
            #   131.0 G - (for DCSC non-AIO)
            #   -------
            #   143.5 G => ~144G min size disk
            # or
            #   151.5 G => ~152G min size disk
            #
            ...
            #      20.0 G - /
            #     123.0 G - cgts-vg PV
            #      96.5 G - unpartitioned free space
            #
            database_storage = \
