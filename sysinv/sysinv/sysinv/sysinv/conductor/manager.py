...
class ConductorManager(service.PeriodicService):
    ...
    def _audit_kubernetes_labels(self, context, hosts):
        ...
        if ((self._admin_token is None) or
                not timeutils.is_expired(self._admin_token)):
            LOG.debug("Retrieving new admin token")
            self._admin_token = self._openstack.req_admin_token()

        context.auth_token = self._admin_token['id']
        LOG.debug("Starting kubernetes label audit")
        sysinv_labels = self.dbapi.label_get_all()
        nodes = self._kube.kube_get_nodes(self._admin_token)
