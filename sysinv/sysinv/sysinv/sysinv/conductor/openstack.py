...
class OpenStackOperator(object):
    ...
    def _set_cached_keystone_session(self, service_config, session):
        if service_config == OPENSTACK_CONFIG:
            self.openstack_keystone_session = session
        else:
            self.keystone_session = session

    def _get_keystone_session(self, service_config):
        session = self._get_cached_keystone_session(service_config)
        if not session:
            session = self._get_new_keystone_session(service_config)
            self._set_cached_keystone_session(service_config, session)
        return session
    ...
    def _get_cinderclient(self):
        if not self.cinder_client:
            self.cinder_client = cinder_client_v2.Client(
                session=self._get_openstack_keystone(OPENSTACK_CONFIG),
                auth_url=self._get_auth_url(OPENSTACK_CONFIG),
                endpoint_type='internalURL',
    ...
    def create_barbican_secret(self, context, name, payload):
        if not payload:
            LOG.error("Empty password is passed to Barbican")
            return None
        client = self._get_barbicanclient()
        ...
        try:
            ...
            LOG.error("Unable to create Barbican secret")
            return None
    ...
    def delete_barbican_secret(self, context, name):
        ...
        if not secret:
            LOG.error("Barbican secret with name %s not found" % name)
            return False
        try:
            client.secrets.delete(secret_ref=secret.secret_ref)
            return True
        except Exception:
            LOG.error("Unable to delete Barbican secret")
            return False
