...
openstack_keystone_opts = [
    cfg.StrOpt('openstack_keyring_service',
               default='CGCS',
...
class OpenStackOperator(object):
    ...
    def _get_keystone_session(self):
        if not self.keystone_session:
            ...
            auth = v3.Password(auth_url=self._get_auth_url(OPENSTACK_CONFIG),
                               username=cfg.CONF[OPENSTACK_CONFIG].username,
                               password=password,
                               user_domain_name=cfg.CONF[OPENSTACK_CONFIG].
                               user_domain_name,
                               project_name=cfg.CONF[OPENSTACK_CONFIG].
                               project_name,
                               project_domain_name=cfg.CONF[OPENSTACK_CONFIG].
                               project_domain_name)
    ...
    def _get_keystoneclient(self, service_config):
        if service_config == OPENSTACK_CONFIG:
            ...
            password = cfg.CONF[PLATFORM_CONFIG].password
