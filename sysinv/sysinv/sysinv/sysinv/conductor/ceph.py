...
class CephOperator(object):
    ...
    def assign_host_to_peer_group(self, host_obj):
        ...
        LOG.error(">>> replication: %s" % replication)
    ...
    def _update_k8s_ceph_pool_secrets(self, ceph_backend):
        """Create CEPH pool secrets for k8s nemaspaces.
        ...
        # This key will be imported in each the K8S secret of each secret.
        ...
        for namespace in namespaces_to_add:
            ...
            # has been creatd for the k8s pool in the given namespace.
        ...
        # Done, store the update capabilities in the ceph_backend reference
    ...
    def check_and_update_rbd_provisioner(self, context, new_storceph=None):
        ...
        changes are needed and only then proceed with more time consuming operations.

        Note: This function assumes a functional Ceph cluster

        :params new_storceph a storoge backend object as_dict() with updated data
                This is required as database updates can happen later.
        ...
        #    1. A cluster wide admin key (e.g. the one in /etc/ceph/ceph.client.admin.keyring)
        #    2. A key for accessing the pool (e.g. client.kube-rbd)
        # - The Ceph keys above passed into Kubernates secrets:
        #    1. An admin secret in the RBD Provisioner POD namespace with
        #       the Ceph cluster wide admin key.
        #    2. One or more K8S keys with the Ceph pool key for each namespace we allow
        #       RBD PV and PVC creations.

        LOG.info("Updating rbd-provisioner configuration.")
        # Manage Ceph cluster wide admin key and associated secret - we create it if needed or
        ...
        for bk in ceph_backends:
            ...
            # Create secret
            # Check to see if we need the admin Ceph key. This key is created once per cluster and
            # references to it are kept in all Ceph tiers of that cluster. So make sure they
            # are up to date.
        ...
        # Remove admin secret and any references to it if RBD Provisioner is unconfigured.
