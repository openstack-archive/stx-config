...
# constants in configcontroller. When it is time to remove/deprecate these
...
def populate_system_config(client):
    ...
    capabilities = {'region_config': False,
                    'vswitch_type': CONF.get(
                        'BOOTSTRAP_CONFIG', 'VSWITCH_TYPE'),
                    'shared_services': '[]',
                    'sdn_enabled': False,
                    'https_enabled': False,
...
def populate_mgmt_network(client):
    ...
    values = {
        ...
        'ranges': [(str(management_subnet[2]),
                    str(management_subnet[-2]))],
    }
...
def populate_oam_network(client):
    ...
    values = {
        ...
        'ranges': [(str(external_oam_subnet[1]),
                    str(external_oam_subnet[-2]))],
...
def populate_multicast_network(client):
    ...
    values = {
        ...
        'ranges': [(str(management_multicast_subnet[1]),
                    str(management_multicast_subnet[-2]))],
    }
    pool = client.sysinv.address_pool.create(**values)
...
def populate_cluster_host_network(client):
    cluster_host_subnet = IPNetwork(CONF.get(
        'BOOTSTRAP_CONFIG', 'CLUSTER_SUBNET'))
    ...
    values = {
        ...
        'ranges': [(str(cluster_host_subnet[2]),
                    str(cluster_host_subnet[-2]))],
    }
    pool = client.sysinv.address_pool.create(**values)
    ...
    values = {
        ...
        'dynamic': True, # Should this be configurable????
        'pool_uuid': pool.uuid,
    }
    client.sysinv.network.create(**values)
...
def populate_cluster_pod_network(client):
    if not INITIAL_POPULATION:
        return
    # Will move to default/main.yml under host_vars in the future
    cluster_pod_subnet = IPNetwork("172.16.0.0/16")
    ...
    values = {
        ...
        'ranges': [(str(cluster_pod_subnet[1]),
                    str(cluster_pod_subnet[-2]))],
    }
    pool = client.sysinv.address_pool.create(**values)
...
def populate_cluster_service_network(client):
    if not INITIAL_POPULATION:
        return
    # Will move to default/main.yml under host_vars in the future
    cluster_service_subnet = IPNetwork("10.96.0.0/12")
    ...
    values = {
        ...
        'ranges': [(str(cluster_service_subnet[1]),
                    str(cluster_service_subnet[-2]))],
    }
    pool = client.sysinv.address_pool.create(**values)
...
def populate_docker_config(client):
    ...
    if not use_default_registries:
        ...
        secure_registry = CONF.getboolean('BOOTSTRAP_CONFIG', 'IS_SECURE_REGISTRY')
        parameters = {}
