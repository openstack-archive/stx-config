...
from configobjects import DEFAULT_NAMES, NETWORK_PREFIX_NAMES, OAM_TYPE, \
    ...
    DEFAULT_DOMAIN_NAME, HP_NAMES, SUBCLOUD_CONFIG, CLUSTER_TYPE
...
class ConfigValidator(object):
    ...
    def validate_cluster(self):
        ...
        if self.cgcs_conf is not None:
            ...
            self.cgcs_conf.set('cCLUSTER', 'CONTROLLER_FLOATING_ADDRESS',
                               str(self.cluster_network.start_address))
            self.cgcs_conf.set('cCLUSTER', 'CONTROLLER_0_ADDRESS',
                               str(self.cluster_network.start_address + 1))
            self.cgcs_conf.set('cCLUSTER', 'CONTROLLER_1_ADDRESS',
                               str(self.cluster_network.start_address + 2))
            ...
            if self.cluster_network.dynamic_allocation:
                ...
                self.cgcs_conf.set('cCLUSTER', 'DYNAMIC_ADDRESS_ALLOCATION',
                                   "no")
            if self.cluster_network.start_address and \
                    self.cluster_network.end_address:
                self.cgcs_conf.set('cCLUSTER', 'CLUSTER_START_ADDRESS',
                                   self.cluster_network.start_address)
                self.cgcs_conf.set('cCLUSTER', 'CLUSTER_END_ADDRESS',
                                   self.cluster_network.end_address)
