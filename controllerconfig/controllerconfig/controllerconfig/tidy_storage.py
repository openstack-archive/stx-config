...
class OpenStack(object):
    ...
    def __init__(self):
        ...
        source_command = 'source /etc/nova/openrc && env'
    ...
    with OpenStack() as client:
        ...
        for snap in snap_l:
            ...
            try:
                ...
                if not keep_snap:
                    print ("Volume snapshot %s has no backend. "
                ...
                LOG.error("Failed to access rbd snapshot for volume %s "
                          % snap.volume_id)
                raise TidyStorageFail("Failed to access rbd snapshot")

            except Exception as e:
                LOG.exception(e)
                raise TidyStorageFail("Failed to delete volume snapshot")
        ...
        for vol_id in in_rbd_volume_only:
            ...
            try:
                ...
                try:
                    for snap in snap_l:
                        # (WEI) Manage a snapshot for a managed volume is
                        #       not supported in rbd.
                        #
        ...
        try:
            c_s_l = c_client.volume_snapshots.list()
