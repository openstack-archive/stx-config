        ...
        try:
            ...
            if self.kubernetes:
                ...
                if config.has_section('cDOCKER_REGISTRY'):
                    ...
                    if config.has_option('cDOCKER_REGISTRY',
                                         'IS_SECURE_REGISTRY'):
                        is_secure_registry = config.get(
                            'cDOCKER_REGISTRY', 'IS_SECURE_REGISTRY').lower()
                        if is_secure_registry == "true":
                            self.is_secure_registry = True
                        elif is_secure_registry == "false":
                            self.is_secure_registry = False
                        else:
                            raise ConfigFail("Invalid value for the option "
                                             "IS_SECURE_REGISTRY.")
                    else:
                        self.is_secure_registry = True
