...
class ConfigAssistant():
    ...
    def __init__(self, labmode=False, kubernetes=False, **kwargs):
        ...
        self.cluster_host_floating_address = IPAddress("192.168.206.2")
        self.cluster_host_address_0 = IPAddress("192.168.206.3")
        self.cluster_host_address_1 = IPAddress("192.168.206.4")
        self.nfs_cluster_host_address_1 = IPAddress("192.168.206.5")
        self.use_entire_cluster_subnet = True
        self.cluster_host_start_address = IPAddress("192.168.206.2")
        self.cluster_host_end_address = IPAddress("192.168.206.254")
    ...
    def input_cluster_host_config(self):
        ...
        while True:
            ...
            if user_input.lower() == 'q':
                ...
                if cluster_vlan_required:
                    print(textwrap.fill(
                        "An cluster host VLAN is required since the "
                        "configured cluster host interface is the "
                        "same as the configured management or external "
                        "OAM interface.", 80))
                    continue
                self.cluster_host_vlan = ""
        ...
        self.cluster_host_start_address = \
            self.cluster_host_subnet[2]
        self.cluster_host_end_address = \
            self.cluster_host_subnet[-2]
        while True:
            user_input = input(
                "Use entire cluster host subnet [Y/n]: ")
            if user_input.lower() == 'q':
                raise UserQuit
            elif user_input.lower() == 'y':
                self.use_entire_cluster_subnet = True
                ...
                self.use_entire_cluster_subnet = False
                break
            elif user_input == "":
                break
            else:
        ...
        if not self.use_entire_cluster_subnet:
            while True:
                while True:
                    user_input = input(
                        "Cluster host network start address [" +
                        str(self.cluster_host_start_address) + "]: ")
                    if user_input.lower() == 'q':
                        raise UserQuit
                    elif user_input == "":
                        user_input = self.cluster_host_start_address
                ...
                while True:
                    user_input = input(
                        "Cluster host network end address [" +
                        str(self.cluster_host_end_address) + "]: ")
                    if user_input.lower() == 'q':
                        raise UserQuit
                    elif user_input == "":
                        user_input = self.cluster_host_end_address

                    try:
                        self.cluster_host_end_address = validate_address_str(
                            user_input, self.cluster_host_subnet)
                        break
                    except ValidateFail as e:
                        print("Invalid cluster host end address. \n"
                              "Reason: %s" % e)

                if not self.cluster_host_start_address < \
                        self.cluster_host_end_address:
                    print("Start address not less than end address. ")
                    print('')
                    continue

                address_range = IPRange(str(self.cluster_host_start_address),
                                        str(self.cluster_host_end_address))
                if not address_range.size >= min_addresses:
                    print(
                        "Address range must contain at least %d addresses. " %
        ...
        default_controller0_cluster_ip = self.cluster_host_start_address + 1
        ip_input = IPAddress(default_controller0_cluster_ip)
        if not self.is_valid_cluster_host_address(ip_input):
            raise ConfigFail("Unable to create controller-0 Cluster host "
                             "address")
        self.cluster_host_address_0 = ip_input
        default_controller1_cluster_ip = \
            self.cluster_host_address_0 + 1
        ip_input = IPAddress(default_controller1_cluster_ip)
        if not self.is_valid_cluster_host_address(ip_input):
            raise ConfigFail("Unable to create controller-1 Cluster host "
                             "address")
        self.cluster_host_address_1 = ip_input

        first_nfs_ip = self.cluster_host_address_1 + 1

        """ create default cluster host NFS address """
        default_nfs_ip = IPAddress(first_nfs_ip)
        if not self.is_valid_cluster_host_address(default_nfs_ip):
            raise ConfigFail("Unable to create NFS cluster host address 1")
        self.nfs_cluster_host_address_1 = default_nfs_ip
    ...
    def input_config_from_file(self, configfile, restore=False):
        ...
        try:
            ...
            if self.kubernetes:
                ...
                self.cluster_host_address_0 = IPAddress(config.get(
                    'cCLUSTER', 'CONTROLLER_0_CLUSTER_ADDRESS'))
                self.cluster_host_address_1 = IPAddress(config.get(
                    'cCLUSTER', 'CONTROLLER_1_CLUSTER_ADDRESS'))
                if config.has_option('cCLUSTER', 'NFS_CLUSTER_HOST_ADDRESS_1'):
                    self.nfs_cluster_host_address_1 = IPAddress(config.get(
                        'cCLUSTER', 'NFS_CLUSTER_HOST_ADDRESS_1'))
                self.cluster_host_interface_configured = True
                if config.has_option('cCLUSTER', 'CLUSTER_START_ADDRESS'):
                    self.cluster_host_start_address = IPAddress(
                        config.get('cCLUSTER',
                                   'CLUSTER_START_ADDRESS'))
                if config.has_option('cCLUSTER', 'CLUSTER_END_ADDRESS'):
                    self.cluster_host_end_address = IPAddress(
                        config.get('cCLUSTER',
                                   'CLUSTER_END_ADDRESS'))
                if not self.cluster_host_start_address and \
                        not self.cluster_host_end_address:
                    self.use_entire_cluster_subnet = True
    ...
    def display_config(self):
        ...
        if self.kubernetes:
            ...
            print("Cluster host floating address: " +
                  str(self.cluster_host_floating_address))
            print("Controller 0 cluster host address: " +
                  str(self.cluster_host_address_0))
            print("Controller 1 cluster host address: " +
                  str(self.cluster_host_address_1))
            print("NFS Cluster Host Address 1: " +
                  str(self.nfs_cluster_host_address_1))
            if not self.use_entire_cluster_subnet:
                print("Cluster host start address: " +
                      str(self.cluster_host_start_address))
                print("Cluster host end address: " +
                      str(self.cluster_host_end_address))
        ...
        try:
            with open(constants.CGCS_CONFIG_FILE, 'w') as f:
                ...
                if self.kubernetes:
                    ...
                    f.write("CLUSTER_FLOATING_ADDRESS=" +
                            str(self.cluster_host_floating_address) + "\n")
                    f.write("CONTROLLER_0_CLUSTER_ADDRESS=" +
                            str(self.cluster_host_address_0)
                            + "\n")
                    f.write("CONTROLLER_1_CLUSTER_ADDRESS=" +
                            str(self.cluster_host_address_1)
                            + "\n")
                    f.write("NFS_CLUSTER_HOST_ADDRESS_1=" +
                            str(self.nfs_cluster_host_address_1) + "\n")
                    f.write("CLUSTER_START_ADDRESS=" +
                            str(self.cluster_host_start_address) + "\n")
                    f.write("CLUSTER_END_ADDRESS=" +
                            str(self.cluster_host_end_address) + "\n")
