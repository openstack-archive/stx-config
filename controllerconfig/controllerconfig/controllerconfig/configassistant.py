...
class ConfigAssistant():
    ...
    def input_docker_proxy_config(self):
        ...
        while True:
            ...
            if user_input.lower() == 'q':
                ...
                while True:
                    user_input = input("HTTP proxy [" +
                                       str(self.docker_http_proxy) + "]: ")
                    if user_input.lower() == 'q':
                        raise UserQuit
                    elif user_input == "":
                        user_input = self.docker_http_proxy
                    self.docker_http_proxy = user_input
                    break

                while True:
                    user_input = input("HTTPS proxy [" +
                                       str(self.docker_https_proxy) + "]: ")
                    if user_input.lower() == 'q':
                        raise UserQuit
                    elif user_input == "":
                        user_input = self.docker_https_proxy
                ...
                while True:
                    ...
                    # net library for proxy setting. The go-lang net lib doesn't
                    # support CIDR notation until this commit:
                    #
                    # https://github.com/golang/net/commit/
                    #     c21de06aaf072cea07f3a65d6970e5c7d8b6cd6d
                    #
                    # After docker upgrades to a version that CIDR notation supported
                    # pre_set_no_proxy will be simplified to subnets.
                    if self.system_mode == sysinv_constants.SYSTEM_MODE_SIMPLEX:
                    ...
                    user_input = input("Additional NO proxy besides '" + pre_set_no_proxy +
                        "'\n(Comma-separated values, wildcard/subnet not allowed)[" +
                        str(self.docker_no_proxy) + "]: ")
                    if user_input.lower() == 'q':
                        raise UserQuit
                    elif user_input == "":
                        user_input = self.docker_no_proxy
                    self.docker_no_proxy = pre_set_no_proxy + "," + user_input
                    break
                break
            elif user_input.lower() in ('n', ''):
                break
        ...
        try:
            with open(constants.CGCS_CONFIG_FILE, 'w') as f:
                ...
                if self.kubernetes:
                    f.write("\n[cDOCKER_PROXY]")
                    f.write("\n# Docker Proxy Configuration\n")
                    f.write("DOCKER_HTTP_PROXY=" +
                        str(self.docker_http_proxy) + "\n")
                    f.write("DOCKER_HTTPS_PROXY=" +
                        str(self.docker_https_proxy) + "\n")
                    f.write("DOCKER_NO_PROXY=" +
                        str(self.docker_no_proxy) + "\n")
    ...
    def _populate_docker_config(self, client):
        ...
        client.sysinv.service_parameter.create('docker', 'proxy', None, None, parameter)
