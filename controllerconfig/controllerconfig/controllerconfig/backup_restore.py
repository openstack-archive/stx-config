...
def get_os_backup_databases():
    ...
    try:
        db_cmd = 'echo "show databases" | '
        db_cmd += 'kubectl --kubeconfig=%s ' % kube_config
        db_cmd += 'exec -i %s -n openstack -- bash ' % mariadb_pod
        db_cmd += '-c \'exec /usr/bin/mysql -uroot -p"$MYSQL_ROOT_PASSWORD"\' '
...
def backup_mariadb_size():
    ...
    try:
        ...
        db_cmd = 'kubectl --kubeconfig=%s ' % kube_config
        db_cmd += 'exec %s -n openstack -- bash ' % mariadb_pod
        db_cmd += '-c \'exec mysqldump -uroot -p"$MYSQL_ROOT_PASSWORD" '
        db_cmd += '--all-databases\' | wc -c'
...
def backup_mariadb(archive, staging_dir):
    ...
    try:
        ...
        for db_elem in os_backup_dbs:
            db_cmd = 'kubectl --kubeconfig=%s ' % kube_config
            db_cmd += 'exec %s -n openstack -- bash ' % mariadb_pod
            db_cmd += '-c \'exec mysqldump -uroot '
            db_cmd += '-p"$MYSQL_ROOT_PASSWORD" %s\' ' % db_elem

            db_cmd += '> %s/%s.sql.data' % (mariadb_staging_dir, db_elem)
...
def extract_mariadb_data(archive):
    ...
    try:
        # We store MariaDB data in /opt/backup-data/mariadb for now.
        # After MariaDB service is up, we will populate the
        # database using these data.
        mariadb_dir = '/opt/backup-data'
        os.mkdir(mariadb_dir)
        archive.extractall(path=mariadb_dir,
...
def restore_mariadb():
    ...
    try:
        ...
        for data in glob.glob(mariadb_staging_dir + '/*.sql.data'):
            db_file = data.split('/')[-1]
            db_elem = data.split('/')[-1].split('.')[0]
            create_db = "create database %s" % db_elem

            # Create the database
            db_cmd = 'echo "%s" ' % create_db
            db_cmd += '| kubectl --kubeconfig=%s ' % kube_config
            db_cmd += 'exec -i %s -n openstack -- bash -c ' % mariadb_pod
            db_cmd += '\'exec /usr/bin/mysql -uroot -p"$MYSQL_ROOT_PASSWORD"\''
            subprocess.check_call([db_cmd], shell=True, stderr=DEVNULL)

            # Populate data
            db_cmd = 'cat %s | kubectl ' % db_file
            db_cmd += '--kubeconfig=%s ' % kube_config
            db_cmd += 'exec -i %s -n openstack -- bash -c ' % mariadb_pod
            db_cmd += '\'exec /usr/bin/mysql -uroot -p"$MYSQL_ROOT_PASSWORD" '
            db_cmd += '%s\' ' % db_elem
            subprocess.check_call([db_cmd], shell=True, stderr=DEVNULL)
...
def check_size(archive_dir):
    ...
    backup_size = (backup_overhead_bytes +
                   ...
                   backup_std_dir_size(constants.HELMCHARTS_PERMDIR) +
                   backup_mariadb_size()
                   )
...
def backup(backup_name, archive_dir, clone=False):
    ...
    try:
        ...
        backup_std_dir(system_archive, constants.HELMCHARTS_PERMDIR)
...
def restore_system(backup_file, include_storage_reinstall=False, clone=False):
    ...
    try:
        ...
        restore_std_dir(archive, constants.HELMCHARTS_PERMDIR)
